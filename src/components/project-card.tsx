...
export default function ProjectCard({
  ...
  return (
    <Link href={`/projects/${projectId}`}>
      <div className="rounded-lg flex flex-col w-full gap-4 bg-zinc-800 p-4 shadow-md">
...
const Pill = ({ text }: { text: string }) => {
  return (
    <div className="rounded-full px-2 py-1 text-xs bg-zinc-500 text-zinc-200 transition-all hover:bg-teal-600/70">
      {text}
    </div>
  );
};
